...
class OlxApp(models.Model):
    ...
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='olx_apps')
    client_domain = models.CharField(max_length=10, choices=CLIENT_DOMAINS)
    client_id = models.CharField(max_length=255)
    client_secret = models.CharField(max_length=255)
    authorization_link = models.URLField(max_length=500, blank=True, null=True, editable=False)
...
class OlxUser(models.Model):
    olxapp = models.ForeignKey(OlxApp, on_delete=models.CASCADE, related_name='olx_users')
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='olx_users', blank=True, null=True)
    bitrix = models.ForeignKey('bitrix.Bitrix', on_delete=models.CASCADE, related_name='olx_users', blank=True, null=True)
    line = models.ForeignKey(Line, on_delete=models.SET_NULL, related_name='olx_users', blank=True, null=True)  # Связь с таблицей Line
    ...
    def add_shedule_task(self):
        ...
        try:
            ...
            if existing_task.interval != interval:
                ...
                logger.info(f"Task '{task_name}' already exists with the correct interval.")
        except PeriodicTask.DoesNotExist:
            # Если задачи с таким именем нет, создаем новую задачу
            PeriodicTask.objects.create(
                name=task_name,
                task='olx.tasks.get_threads',
                interval=interval,
