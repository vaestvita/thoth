...
def refresh_token(olx_user_id):
    ...
    if get_token.status_code == 200:
        ...
        logger.error(f'Failed to refresh token for user {user.olx_id}. Status code: {get_token.status_code}, Response: {get_token.text}')
...
def send_message(chat_id, text, files=None):
    ...
    user = OlxUser.objects.get(olx_id=olx_user_id)
    olx_app = user.olxapp
    api_url = f"https://www.{olx_app.client_domain}/api/partner/threads/{threadid}/messages"
    ...
    if response.status_code == 401:
        if refresh_token(olx_user_id):
            ...
            logger.error(f"Failed to refresh token for user {olx_user_id} after receiving 401.")
