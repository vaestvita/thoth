...
def get_threads(olx_user_id):
    try:
        ...
        if response.status_code == 200:
            ...
            for thread in threads:
                ...
                if unread_count != 0:
                    ...
                    if messages_response.status_code == 200:
                        ...
                        logger.debug(f"Received {len(messages)} messages for thread {thread_id}")

                        # Отбираем последние непрочитанные сообщения с типом "received"
                        received_messages = [msg for msg in messages if msg['type'] == 'received']
                        unread_messages = sorted(received_messages, key=lambda x: x['created_at'])[-unread_count:]

                        # Отправляем выбранные непрочитанные сообщения
                        for msg in unread_messages:
                            logger.debug(f"Unread Received Message in thread {thread_id}: {msg}")
                            ...
                            call_method(user.bitrix, 'imconnector.send.messages', payload)
                        ...
                        logger.error(f"Failed to retrieve messages for thread {thread_id}. "
            ...
            logger.error(f"Failed to retrieve threads for user {user.olx_id}. "
        ...
        logger.error(f"An error occurred while processing OLX threads for user {olx_user_id}: {str(e)}")
