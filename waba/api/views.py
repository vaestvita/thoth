...
class WabaWebhook(GenericViewSet, CreateModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if hub_mode == 'subscribe' and hub_verify_token:
            try:
                waba = Waba.objects.get(verify_token=hub_verify_token, owner=request.user.id)
                return HttpResponse(hub_challenge, content_type='text/plain')
            except Waba.DoesNotExist:
                logger.error(f'Verification token not found or does not belong to the user {request.query_params}')
                return HttpResponse('token not found', status=403, content_type='text/plain')
